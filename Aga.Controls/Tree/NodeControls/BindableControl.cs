...
namespace Aga.Controls.Tree.NodeControls
{
	public abstract class BindableControl : NodeControl
	{
        ...
        private MemberAdapter GetMemberAdapterOfTag(Object tag)
        {
            if (tag == null)
            ...
            if (pi != null)
                return new MemberAdapter(tag, pi);

            FieldInfo fi = type.GetField(DataPropertyName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            if (fi != null)
                return new MemberAdapter(tag, fi);
            
            if(tag is Tree.Node)
            {
                return GetMemberAdapterOfTag(((Tree.Node)tag).Tag);
            }
        ...
        private MemberAdapter GetMemberAdapter(TreeNodeAdv node)
		{
			...
			{
                MemberAdapter found = GetMemberAdapterOfTag(node.Tag);
